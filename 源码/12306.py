...
class hackTickets(object):
    ...
    def readConfig(self, config_file='config.ini'):
        ...
        try:
            ...
            cp.read_file(open(config_file, "r", "utf-8-sig"))
        except IOError as e:
    ...
    def sendmail(self,subject,message):
        ...
        # 使用你的邮件账户名和密码登录服务器
        t_server = server(self.mail_user, self.mail_pwd)
        # 发送邮件指令
        t_server.send_mail([self.receiver], mail_content)
    """入口函数"""
    def start(self):
        # 初始化驱动
        self.driver=Browser(driver_name=self.driver_name,executable_path=self.executable_path)
        # 初始化浏览器窗口大小
#        self.driver.driver.set_window_size(1366, 768)
